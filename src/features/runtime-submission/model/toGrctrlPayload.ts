...
const NUMERIC_SCALAR_PATTERN =
	/^-?(?:(?:\d+\.?\d*)|(?:\.\d+))(?:[eE][+-]?\d+)?$/;

function shouldRenderAsFloatScalar(
	parameterMeta: BlockParameterMeta | undefined,
): boolean {
	const normalized = parameterMeta?.valueType?.trim().toLowerCase() ?? "";
	if (
		!normalized ||
		normalized.includes("vector") ||
		normalized.includes("tensor")
	) {
		return false;
	}
...
function yamlTagForParameter(
	...
	if (normalized === "float_vector") {
		...
		return parameterMeta && knownDoubleVectorParameters.has(parameterMeta.name)
			? "!!float64"
			: undefined;
	}
...
function getLiteralStringParameter(
...
function expandVirtualRoutes(params: {
	...
	const virtualNodes = params.nodes.filter((node) =>
		isVirtualRoutingBlockType(node.blockType),
	);
	...
	virtualNodes.forEach((node) => {
		...
		if (!streamId) {
			throw new Error(
				`Virtual routing block "${node.id}" requires a literal stream_id parameter.`,
			);
		}
	...
	const nextEdges = params.edges.filter(
		(edge) =>
			!virtualNodeIds.has(edge.source.nodeId) &&
			!virtualNodeIds.has(edge.target.nodeId),
	);
	...
	for (const [streamId, sources] of virtualSourcesByStreamId.entries()) {
		...
		if (!sink) {
			throw new Error(
				`Virtual source route "${streamId}" has no matching virtual sink.`,
			);
		}

		const incoming = params.edges.filter(
			(edge) => edge.target.nodeId === sink.id,
		);
...
function buildRuntimeGraph(document: GraphDocument): {
	...
	const omittedEditorNodeIds = new Set(
		document.graph.nodes
			.filter((node) => isNoteBlockType(node.blockType))
			.map((node) => node.id),
	);
...
function shouldOmitParameterForBlock(
	blockTypeId: string,
	name: string,
	rawValue: JsonPrimitive | undefined,
): boolean {
...
function serializeGraphDocumentToInlineGrc(
	...
	const nodes = [...runtimeGraph.nodes].sort((left, right) =>
		left.id.localeCompare(right.id),
	);
	const edges = [...runtimeGraph.edges].sort((left, right) =>
		left.id.localeCompare(right.id),
	);
	...
	lines.push(
		...indent([
			`description: ${sanitizeScalar(document.metadata.description ?? "")}`,
		]),
	);
	lines.push(`blocks:`);
		...
		nodes.forEach((node) => {
			...
			if (blockDetails) {
				blockDetails.parameters.forEach((parameter) => {
					if (
						parameter.name === "name" ||
						parameterEntries.has(parameter.name)
					) {
						return;
					}
			...
			if (studioBinding) {
				...
				// Export the compatibility payload format and transport explicitly so
				// legacy graphs and graph documents without block-details hydration still
				// prepare cleanly.
				if (!parameterEntries.has("payload_format")) {
					parameterEntries.set("payload_format", {
						kind: "literal",
						value: studioBinding.payloadFormat,
					});
				}
				if (!parameterEntries.has("transport")) {
					parameterEntries.set("transport", {
						kind: "literal",
						value: "http_poll",
					});
				}
			...
			const sortedParameterEntries = [...parameterEntries.entries()].sort(
			...
			if (sortedParameterEntries.length > 0) {
				sortedParameterEntries.forEach(([name, parameter]) => {
					...
					const resolvedParameter =
						resolved.parametersByNodeId[node.id]?.[name];
					const fallbackValue =
						parameter.kind === "literal" ? parameter.value : undefined;
					const parameterValue =
						resolvedParameter?.state === "resolved"
							? resolvedParameter.value
							: fallbackValue;
					if (
						shouldOmitParameterForBlock(node.blockType, name, parameterValue)
					) {
						return;
					}
					const parameterMeta = blockDetails?.parameters.find(
						(candidate) => candidate.name === name,
					);
					lines.push(
						...indent([
							`    ${name}: ${renderParameterValue(name, parameterValue, parameterMeta)}`,
						]),
					);
				});
			}
